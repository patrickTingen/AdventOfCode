...
DEFINE VARIABLE iRam AS INT64 EXTENT INITIAL [{2019-09-1.dat}] NO-UNDO.
...
FUNCTION showMemory RETURNS CHARACTER():
  DEFINE VARIABLE i AS INTEGER   NO-UNDO.
  DEFINE VARIABLE c AS CHARACTER NO-UNDO.
  DO i = 1 TO EXTENT(iRam):
    c = SUBSTITUTE('&1[&2] ', c, iRam[i]).
  END.
  RETURN c.
END FUNCTION.
...
FUNCTION runProgram RETURNS LOGICAL (
  ...
  DO WHILE TRUE:
    ...
    DISPLAY cOpcode cmode1 cmode2 cmode3
      WITH FRAME f DOWN. DOWN WITH FRAME f.
