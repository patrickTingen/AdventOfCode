/*------------------------------------------------------------------------
    File        : 2017-11.p
    Purpose     : Day 11 of AoC 2017 

    24-12-2017 Patrick Tingen
  ----------------------------------------------------------------------*/

FUNCTION combine RETURNS LONGCHAR
  ( str AS LONGCHAR, s1 AS CHAR, s2 AS CHAR, s3 AS CHAR):
...
MESSAGE NUM-ENTRIES(s) STRING(s) /* 805 */
  VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.

/* Attempt 2: hex calculus */
RUN getData(OUTPUT s).

DO i = 1 TO NUM-ENTRIES(s):
  CASE ENTRY(i,s):
    WHEN "n"  THEN ASSIGN y = Y + 1 z = z - 1.
    WHEN "s"  THEN ASSIGN Y = Y - 1 z = z + 1.
    WHEN "ne" THEN ASSIGN X = X + 1 z = z - 1.
    WHEN "sw" THEN ASSIGN X = X - 1 z = z + 1.
    WHEN "nw" THEN ASSIGN X = X - 1 Y = Y + 1.
    WHEN "se" THEN ASSIGN X = X + 1 Y = Y - 1.
  END CASE.
  
    m = MAX(m , ((ABS(X) + ABS(Y) + ABS(z)) / 2)).
END.

MESSAGE ((ABS(X) + ABS(Y) + ABS(z)) / 2) /* 805 */
        SKIP 'Max:' m  /* 1535 */
  VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.

/* attempt 3: x,y system */
RUN getData(OUTPUT s).

DO i = 1 TO NUM-ENTRIES(s):
  CASE ENTRY(i,s):
    WHEN "n"  THEN ASSIGN Y = Y + 2.
    WHEN "ne" THEN ASSIGN X = X + 1 Y = Y + 1.
    WHEN "nw" THEN ASSIGN X = X - 1 Y = Y + 1.
    WHEN "s"  THEN ASSIGN Y = Y - 2.
    WHEN "se" THEN ASSIGN X = X + 1 Y = Y - 1.
    WHEN "sw" THEN ASSIGN X = X - 1 Y = Y - 1.
  END CASE.
END. 

MESSAGE (ABS(X) + ABS(Y)) / 2   /* 1433 (huh?) */
  VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.

/* Attempt 4: Code from Jean Christophe */
...
RUN getData(OUTPUT cInput).

DO iMove = 1 TO NUM-ENTRIES(cInput):
    cMove = ENTRY(iMove, cInput).
    CASE cMove:
        WHEN "n"  THEN ASSIGN
            iy = iy + 1.
        WHEN "ne" THEN ASSIGN
        ...
        WHEN "nw" THEN ASSIGN
            ...
            iy = iy + 1.
        OTHERWISE
            MESSAGE "illegal move " + QUOTER(cMove)
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
    END CASE.
END.

/* using a 3 x,y,z coordinate system */
iz = (ix + iy) * -1.

MESSAGE MAXIMUM(ix,iy,iz) SKIP ix iy iz /* 805 */
    VIEW-AS ALERT-BOX INFO BUTTONS OK.
