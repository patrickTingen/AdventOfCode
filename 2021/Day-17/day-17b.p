...
FUNCTION hitTarget RETURNS INTEGER
  ( piStartX AS INTEGER
...
DO xx = 1 to giTargetX[2]:
  DO yy = MIN(giTargetY[1], giTargetY[2]) TO MAX(ABS(giTargetY[1]), ABS(giTargetY[2])):
    ...
    hh = hh + hitTarget(xx, yy).
    
  END.
END.

MESSAGE hh VIEW-AS ALERT-BOX INFORMATION BUTTONS OK. /* 1733 */
...
FUNCTION hitTarget RETURNS INTEGER
  ...
  REPEAT:
    ...
    IF    iPosX >= giTargetX[1] 
      ...
      AND iPosY <= giTargetY[2] THEN RETURN 1.
